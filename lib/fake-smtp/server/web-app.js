...
p._configureRoutes = function() {
  ...
  app.post('/api/emails/:uuid/transfer', bodyParser.json(), this._transferEmail.bind(this));
...
p._transferEmail = function(req, res) {
  ...
  mailStream.once('end', function(email) {
    ...
    var transporter = nodemailer.createTransport(config.transferConfig);
    ...
    transporter.sendMail({
      ...
      subject: '[Faketools Transfer] '+ email.subject
    }, function(err, info){
      if(err) {
        console.error(err.stack);
        res.status(500).end();

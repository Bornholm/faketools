...
p.start = function(cb) {
  ...
  var httpsOpts = config.cas.httpsOpts;

  if(typeof cb !== 'function') cb = function(err) { if(err) throw err };

  // Load cert if file path defined
  try {
    if(httpsOpts.certPath) httpsOpts.cert = fs.readFileSync(httpsOpts.certPath);
  } catch(err) {
    return cb(err);
  }

  try {
    if(httpsOpts.keyPath) httpsOpts.key = fs.readFileSync(httpsOpts.keyPath);
  } catch(err) {
    return cb(err);
  }

  https.createServer(config.cas.httpsOpts, this._app)
    .listen(config.cas.port, config.cas.host, function(err) {
      if(err) {
        console.error(err);
        return cb(err);
      }
      console.log('Server listening on https://%s:%s', config.cas.host, config.cas.port);
      return cb();
    })
  ;
};
...
p._handleLoginForm = function(req, res) {
  ...
  this._findUser(login, function(err, user) {
    ...
    if(!user || !(user.password && user.password === password)) {
      return res.redirect(303, '/login?err=Utilisateur inconnu ou mot de passe invalide.');
    }
...
p._ticketExists = function(service, userId) {
  var t = this._tickets;
  var hash = this._getServiceHash(service);
  return t[hash] ? ( t[hash][userId] ? true : false ) : false;
};

p._getServiceHash = function(service) {
...
p._getServiceTicket = function(service, userId) {
  var hash = this._getServiceHash(service);
  var tickets = this._tickets[hash] = this._tickets[hash] || {};
...
p._findUserForServiceTicket = function(service, ticket, cb) {
  ...
  var hash = this._getServiceHash(service);
  var tickets = this._tickets[hash];
